...
def generate(prompt, args, tokenizer, model=None, device=None):
    ...
    if args.model_name_or_path in API_MODEL_MAP:
        ...
        # next_token_logits = model(tokd_input["input_ids"])
        if args.use_sampling:
...
def generate_with_low_entropy(prompt, args, tokenizer, model=None, device=None):
    """Manually implement generation process instead of using model default generation """
    ...
    if args.model_name_or_path in API_MODEL_MAP:
        ...
        input_ids_without = tokd_input["input_ids"]
        # input_ids_without = model(tokd_input["input_ids"])
        torch.manual_seed(args.generation_seed)
        ...
        with_stop, without_stop = False, False
        ...
        for i in range(max_new_tokens):
            ...
            if not with_stop:
                ...
                logits = watermark_processor(input_ids, logits)
                if temperature != 1.0:
                    logits /= temperature
                ...
                # Break the loop if the generated token is an end-of-sequence token
                if next_token.item() == tokenizer.eos_token_id or count > max_new_tokens-1:
                    with_stop = True
                else:
                    single_logits = model(next_token).logits
                    single_logits = single_logits[:,-1,:]
                    single_logits = watermark_processor(next_token, single_logits)

                    if temperature != 1.0:
                        single_logits /= temperature
                    
                    single_probabilities = torch.nn.functional.softmax(single_logits,dim=-1)
                    max_single_prob, next_token = torch.max(single_probabilities, dim=-1)
                    next_token = torch.unsqueeze(next_token, dim=0)

                    if max_single_prob >= args.low_entropy_prob:

                        tmp_input_ids = copy.deepcopy(input_ids)
            ...
            if not without_stop:
                with torch.no_grad():
                    out = model(input_ids_without)
                    logits_without_watermark = out.logits[:,-1,:]

                if temperature != 1.0:
                    logits_without_watermark /= temperature
                
                # Softmax to get probabilities
                probabilities_without_watermark = torch.nn.functional.softmax(logits_without_watermark, dim=-1)

                # Sample the next token
                next_token_without_watermark = torch.multinomial(probabilities_without_watermark, 1)
                input_ids_without = torch.cat([input_ids_without, next_token_without_watermark], dim=-1)

                # Update input_ids for the next iteration
                if i==0:
                    generated_token_without = next_token_without_watermark
                else:
                    generated_token_without = torch.cat([generated_token_without,next_token_without_watermark], dim=-1)

                # Break the loop if the generated token is an end-of-sequence token
                if next_token_without_watermark.item() == tokenizer.eos_token_id:
                    without_stop = True
                # else:
        ...
        generated_text_without_watermark = tokenizer.batch_decode(generated_token_without, skip_special_tokens=True)[0]
        all_without_words, all_with_words = "", ""
        for without_word, with_word in zip(generated_text_without_watermark.split(), generated_text.split()):
            all_without_words += without_word + " "
            all_with_words += with_word + " "
            yield all_without_words, all_with_words
        
            # yield generated_text_without_watermark, generated_text

def distribution_comparision(tmp_input_ids, next_token, model,divergence_threshold):
    with torch.no_grad():
        out = model(tmp_input_ids)
        logits = out.logits[:,-1,:]
 
    original_probs = torch.nn.functional.softmax(logits, dim=-1)

    # for logit,prob in zip(logits[0], original_probs[0]):
    #     print(logit,prob)
    
    tmp_input_ids = torch.cat([tmp_input_ids, next_token], dim=-1)
    with torch.no_grad():
        second_out = model(tmp_input_ids)
        modified_logits = second_out.logits[:,-1,:]
    modified_probs = torch.nn.functional.softmax(modified_logits, dim=-1)
    
    # Clip values to avoid NaN in logarithm
    epsilon = 1e-7
    original_probs += epsilon
    modified_probs += epsilon

    # Calculate Kullback-Leibler (KL) divergence
    kl_divergence = torch.sum(original_probs * torch.log(original_probs / modified_probs))
    print(kl_divergence)
    return kl_divergence.item() <= divergence_threshold
...
def run_gradio(args, model=None, device=None, tokenizer=None):
    ...
    generate_low_partial = partial(generate_with_low_entropy, model=model, device=device)
    ...
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Tab("Generate & Detect"):
            ...
            with gr.Row():
                generate_low_entro_btn = gr.Button("Generate with more low-entropy words")
            with gr.Row():
                with gr.Column(scale=2):
        ...
        with gr.Accordion("Advanced Settings",open=False):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        low_entropy_prob = gr.Slider(label="low-entropy probability", minimum=0.5, maximum=1.0, step=0.01, value=args.low_entropy_prob)
                    with gr.Row():
                        divergence_threshold = gr.Slider(label=" divengence threshold", minimum=0.01, maximum=10, step=0.01, value=args.divergence_threshold)
        ...
        with gr.Accordion("What do the settings do?",open=False):
            ...
            - **Low entropy probability** : When generating single tokens, one of the conditions for choosing this token is that
                                            if the probability after softmax function applied, is greater than this value.            
            - **Divergence thershold** : Kullback-Leibler (KL) divergence is used to compare the simularity of two probability distributions (The result with new token added vs without added)
        ...
        #Same as above
        generate_low_entro_btn.click(fn=check_prompt_partial, inputs=[prompt,session_args,session_tokenizer], outputs=[redecoded_input, truncation_warning, session_args]).success(
                           fn=generate_low_partial, inputs=[redecoded_input,session_args,session_tokenizer], outputs=[output_without_watermark, output_with_watermark]).success(
                           fn=detect_partial, inputs=[output_without_watermark,session_args,session_tokenizer], outputs=[without_watermark_detection_result,session_args,session_tokenizer,html_without_watermark]).success(
                           fn=detect_partial, inputs=[output_with_watermark,session_args,session_tokenizer], outputs=[with_watermark_detection_result,session_args,session_tokenizer,html_with_watermark])
        ...
        def update_low_entropy_prob(session_state, value): session_state.low_entropy_prob = value; return session_state
        def update_divergence_threshold(session_state, value): session_state.divergence_threshold = value; return session_state
        ...
        low_entropy_prob.change(update_low_entropy_prob,inputs=[session_args, low_entropy_prob], outputs=[session_args])
        divergence_threshold.change(update_divergence_threshold,inputs=[session_args, divergence_threshold], outputs=[session_args])
        ...
        generate_low_entro_btn.click(lambda value: str(value), inputs=[session_args], outputs=[current_parameters])
...
def main(args): 
    ...
    if not args.skip_model_load:
        ...
        generator_outputs = generate_with_low_entropy(input_text, 
                                    args, 
                                    model=model, 
                                    device=device, 
                                    tokenizer=tokenizer)
